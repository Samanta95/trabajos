...
var app = {
    init: function() {
        ...
        if (!isMobile.any) {
            _top_filtros = $("#banner").offset().top;
            offset_filtros = _top_filtros - MARKETING_RIBBON.height();
            $(window).scroll(function() {
                var fromTop = $(this).scrollTop();
                if (fromTop > offset_filtros) {
                    $("#seccion-principal").addClass("fixed").css({
                        top: MARKETING_RIBBON.height()
                    });
                } else {
                    $("#seccion-principal").removeClass("fixed").removeAttr("style");
                }
            });
        } else {
        ...
        $(window).scroll(function(event) {
            var fromTop = $(this).scrollTop();
               var indice = $(".section").map(function(index, elem) {
                   if (fromTop > $(this).offset().top - $(".menu").innerHeight()) {
                       return index;
                   }
               });
            $(".menu ul a").removeClass("active");
            $(".menu ul a").eq(indice[indice.length - 1]).addClass("active");
        });
        this.sliders();
    },
    scrolling: function(_hash, _speed) {
        ...
        if ($(_hash).length > 0) {
            $("html, body").animate({
                scrollTop: $(_hash).offset().top-60
            }, _speed, function() {
                var item = $(".menu ul a").map(function(index, elem) {
                    if ($(this).attr("href") == _hash) {
                        return index;
                    }
                });
                $(".menu ul a").removeClass("active");
                $(".menu ul a").eq(item[0]).addClass("active");
            });
        }
        return false;
    },
...
window.onload = function() {
    ...
    $(".play").click(function(e) {
        player.playVideo(), $(this).parent().fadeOut(), console.log("playing")
    })
};
