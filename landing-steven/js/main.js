...
var app = {
    init: function() {
        ...
        if (!isMobile.any) {
            ...
            _top_filtros = $("#menu-mobile").offset().top;
            offset_filtros = _top_filtros - MARKETING_RIBBON.height();
            $(window).scroll(function() {
                var fromTop = $(this).scrollTop();
                if (fromTop > offset_filtros) {
                    $("#menu-mobile").addClass("fixed").css({
                        top: MARKETING_RIBBON.height()
                    });
                } else {
                    $("#menu-mobile").removeClass("fixed").removeAttr("style");
                }
            });
        ...
        $(window).scroll(function(event) {
          ...
          $("#parte-2 ul a").removeClass("active");
          $("#parte-2 ul a").eq(indice[indice.length - 2]).addClass("active");
      });
        this.sliders();
    },
    scrolling: function(_hash, _speed) {
        ...
        if ($(_hash).length > 0) {
            $("html, body").animate({
                ...
                var item = $("#parte-2 ul a").map(function(index, elem) {
                    if ($(this).attr("href") == _hash) {
                        return index;
                    }
                });
                $("#parte-2 ul a").removeClass("active");
                $("#parte-2 ul a").eq(item[0]).addClass("active");
            });
        }
        return false;
    },
...
//$(document).ready(function () {
//    $('#landing-blackanddecker #hover ').hover(
//         function(){
//             $(this).children().attr('src','./images/herramienta-'+$(this).children().attr('attr-id')+'-h.jpg');
//         },
//         function(){
//             $(this).children().attr('src','./images/herramienta-'+$(this).children().attr('attr-id')+'.jpg');
//         }
//     );
// });
